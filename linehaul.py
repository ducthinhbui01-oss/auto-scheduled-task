...
def run():
    ...
    try:
        ...
        # BƯỚC 3: KÉO ĐA LUỒNG TẤT CẢ CÁC TRANG CỦA TẤT CẢ CÁC HUB
        ...
        total_trips = p1_res.get('data', {}).get('total', 0) if isinstance(p1_res.get('data'), dict) else 0
        p1_trips = p1_res.get('data', {}).get('list') or p1_res.get('data', {}).get('trips') or []
        
        all_trips = list(p1_trips)
        total_pages = min(math.ceil(total_trips / 100), 50) if total_trips > 0 else 1
